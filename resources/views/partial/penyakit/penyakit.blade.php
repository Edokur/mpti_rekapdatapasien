                ...
                <div class="p-4">
                    <form class="input-group mb-3" action="/penyakit/cari" method="GET">
                        ...
                        <div class="input-group-append">
                            <a href="/penyakit" class="btn btn-warning">
                ...
                <div class="p-4">
                    <table class="table table-bordered rounded">
                        ...
                        @foreach($data as $key=>$value)
                            <tr>
                                <td class="col-1 p-3">{{ $loop->iteration }}</td>
                                <td class="col-1 p-3">{{ $value->id_register}}</td>
                                <td class="col-2 p-3">{{ $value->nama_penyakit}}</td>
                                <td class="col-1 p-3">{{ $value->tanggal}}</td>
                                <td class="col-4 p-3">{{ $value->deskripsi}}</td>
                                <td class="col-2 td1 border">
                                    <a href="/penyakit/editPenyakit/{{ $value->id_identitas_penyakit}}" class="btn">
                                    ...
                                    <a href="/penyakit/detailPenyakit/{{ $value->id_identitas_penyakit}}" class="btn">
                                    ...
                                    <button data-id="{{ $value->id_identitas_penyakit}}" data-name="{{ $value->nama_penyakit }}" type="submit" class="btn btn-hapusPenyakit">
...
<script>
    $('.btn-hapusPenyakit').click(function() {
        var id_identitas_penyakit = $(this).attr('data-id');
        var nama_penyakit = $(this).attr('data-name');
        swal({
                title: "Apakah Anda Yakin?",
                text: "Kamu akan menghapus data dengan Nama " + nama_penyakit + " ",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            .then((willDelete) => {
                if (willDelete) {
                    window.location = "/penyakit/hapusPenyakit/" + id_identitas_penyakit + "";
